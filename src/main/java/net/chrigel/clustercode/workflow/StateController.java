...
import net.chrigel.clustercode.workflow.actions.InitializeAction;
...
public class StateController
    ...
    public void initialize() {
        ...
        StateMachineBuilder<StateController, WorkflowState, WorkflowEventType, StateContext> builder =
                StateMachineBuilderFactory.create(getClass(), WorkflowState.class, WorkflowEventType.class, StateContext
                        .class);
        ...
        builder.externalTransition().from(WorkflowState.WAIT).to(WorkflowState.SCAN_MEDIA)
                .on(WorkflowEventType.TIMEOUT);

        builder.externalTransition().from(WorkflowState.SCAN_MEDIA).to(WorkflowState.WAIT)
                .on(WorkflowEventType.NO_RESULT);
        builder.externalTransition().from(WorkflowState.SCAN_MEDIA).to(WorkflowState.SELECT_MEDIA)
                .on(WorkflowEventType.RESULT);

        builder.externalTransition().from(WorkflowState.SELECT_MEDIA).to(WorkflowState.PARSE_PROFILE)
                .on(WorkflowEventType.RESULT);
        builder.externalTransition().from(WorkflowState.WAIT).to(WorkflowState.PARSE_PROFILE)
                .on(WorkflowEventType.NO_RESULT);

        builder.externalTransition().from(WorkflowState.PARSE_PROFILE).to(WorkflowState.TRANSCODE)
                .on(WorkflowEventType.RESULT);
        builder.externalTransition().from(WorkflowState.TRANSCODE).to(WorkflowState.CLEANUP)
                .on(WorkflowEventType.FINISHED);
        builder.externalTransition().from(WorkflowState.CLEANUP).to(WorkflowState.SCAN_MEDIA)
                .on(WorkflowEventType.FINISHED);
